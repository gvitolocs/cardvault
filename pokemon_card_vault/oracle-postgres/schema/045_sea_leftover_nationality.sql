-- SEA leftover faces (visual, 2026-09-11):
-- Scarlet & Violet Indonesian Promos — Eevee 074/SV-P and Fanfare Perayaan
-- print Indonesian body text (Kelemahan / Mundur / Perayaan). Not EN, not JP.
-- Thailand & Indonesia Products — mixed: Eevee 228/SV-P Indonesian and
-- Eevee 225/SV-P Thai (อีวุย). Expansion name is Products → product (no flag).
--
-- Previous thailand|indonesia → unknown hid both from the set-title flag.
-- Do not mark Indonesian leftovers western / EUUS.
...
as $fn$
  select case
    ...
    when lower(coalesce(name, '')) = 'thailand & indonesia products' then 'product'
...
update public.pokoin_pokemon_expansions
set nationality = 'product',
    milo_gallery = public.pokoin_expansion_milo_gallery('product', kind),
    updated_at = now()
...
where lower(name) in (
  'scarlet & violet indonesian promos',
