...
drop function if exists public.search_marketplace_blueprint_name_candidates(text, integer, integer, text);
drop function if exists public.search_marketplace_blueprint_non_name_candidates(text, integer, integer, text);
drop function if exists public.search_marketplace_blueprint_candidates_v2(text, integer, integer);
...
as $$
      ...
      exists (
        ...
        where not qt.token ~ '^[0-9]+$'
          ...
          and (
            ...
            or exists (
              select 1
              from public.marketplace_card_names_for_language((select language from normalized)) n
              where n.normalized_name = qt.token
                or n.compact_name = public.marketplace_search_compact(qt.token)
                or qt.token = any(n.name_tokens)
            )
          )
      ) as has_text_token
  ),
  name_hits as (
    select
      ...
      (
        case
          ...
          when (length(qt.token) >= 2 or qt.token = 'n') and n.normalized_name like qt.token || '%' then 1040
          when (length(qt.token) >= 2 or qt.token = 'n') and n.compact_name like public.marketplace_search_compact(qt.token) || '%' then 980
          when length(qt.token) between 3 and 8
    ...
    join public.marketplace_card_names_for_language(norm.language) n
      on n.normalized_name = qt.token
      or n.compact_name = public.marketplace_search_compact(qt.token)
      or ((length(qt.token) >= 2 or qt.token = 'n') and n.normalized_name like qt.token || '%')
      or ((length(qt.token) >= 2 or qt.token = 'n') and n.compact_name like public.marketplace_search_compact(qt.token) || '%')
      or (
        length(qt.token) between 3 and 8
        and abs(length(n.compact_name) - length(public.marketplace_search_compact(qt.token))) <= 1
        and left(n.compact_name, 2) = left(public.marketplace_search_compact(qt.token), 2)
  ...
  name_word_hits as (
    ...
    cross join lateral (
      ...
      from regexp_split_to_table(public.marketplace_search_normalize(coalesce(nullif(c.canonical_name, ''), c.name)), ' ') word_value
      where word_value <> ''
    ) name_word
  ...
  rarity_alias_hits as (
    select
      ...
      (
        case
          ...
          when qt.token in ('ir', 'ill', 'illus', 'illustration')
    ...
    join public.marketplace_search_candidates c
      ...
      or (
        qt.token in ('ir', 'ill', 'illus', 'illustration')
  ...
  trainer_hits as (
    select
      qt.token as query_token,
      'trainer'::text as token_kind,
      c.card_id as entity_key,
      (
        case
          when public.marketplace_search_normalize(c.trainer_name) = qt.token then 900
          when qt.token = any(public.marketplace_search_tokenize(c.trainer_name)) then 860
          when public.marketplace_search_compact(c.trainer_name) = public.marketplace_search_compact(qt.token) then 840
          when public.marketplace_search_normalize(c.trainer_name) like qt.token || '%' then 700
          when public.marketplace_search_compact(c.trainer_name) like public.marketplace_search_compact(qt.token) || '%' then 680
          else 0
        end
      )::numeric as token_score
    from query_tokens qt
    join public.marketplace_search_candidates c
      on c.trainer_name <> ''
      and (
        public.marketplace_search_normalize(c.trainer_name) = qt.token
        or qt.token = any(public.marketplace_search_tokenize(c.trainer_name))
        or public.marketplace_search_compact(c.trainer_name) = public.marketplace_search_compact(qt.token)
        or public.marketplace_search_normalize(c.trainer_name) like qt.token || '%'
        or public.marketplace_search_compact(c.trainer_name) like public.marketplace_search_compact(qt.token) || '%'
      )
  ),
  variation_hits as (
    select
  ...
  token_blueprints as (
    ...
    from name_hits h
    join public.marketplace_search_candidates c on coalesce(nullif(c.canonical_name, ''), c.name) = h.entity_key
    union all
    ...
    join public.marketplace_search_candidates c on c.card_id = h.entity_key
    union all
    select c.card_id, h.query_token, h.token_kind, h.entity_key::text, h.token_score + 500 + c.search_weight as score
    from trainer_hits h
    join public.marketplace_search_candidates c on c.card_id = h.entity_key
    union all
      ...
      bool_or(
        ...
        and (
          ...
          or exists (
            select 1
            from public.marketplace_card_names_for_language((select language from normalized)) n
            where n.normalized_name = query_token
              or n.compact_name = public.marketplace_search_compact(query_token)
              or query_token = any(n.name_tokens)
          )
        )
      ) as matched_name,
      bool_or(token_kind = 'number') as matched_number,
      bool_or(token_kind = 'variation') as matched_variation,
      bool_or(token_kind = 'trainer') as matched_trainer,
  ...
  scored_cards as (
    select
      ...
      coalesce(sum(p.token_score), 0)
        + case
            when public.marketplace_search_normalize(coalesce(nullif(c.canonical_name, ''), c.name)) = (select q from normalized) then 2600
            when public.marketplace_search_compact(coalesce(nullif(c.canonical_name, ''), c.name)) = (select compact_q from normalized) then 2400
            else 0
          end
        ...
        + case
            when bool_or(p.matched_name) and bool_or(p.matched_trainer) then 700
            else 0
          end
        + case
...
create or replace function public.search_marketplace_blueprint_name_candidates(
  search_term text,
  result_limit integer default 20,
  result_offset integer default 0,
  search_language text default 'en'
...
as $$
  ...
  query_tokens as (
    ...
    where token <> ''
      ...
      and not exists (
        select 1
        from public.marketplace_variations v
        where token = any(v.normalized_aliases)
          or public.marketplace_search_compact(token) = any(v.compact_aliases)
      )
  ),
  name_hits as (
    select
  ...
  token_blueprints as (
    select c.card_id, h.query_token, h.token_score + 640 + c.search_weight as score
    from name_hits h
    join public.marketplace_search_candidates c on coalesce(nullif(c.canonical_name, ''), c.name) = h.entity_key
    union all
    select c.card_id, h.query_token, h.token_score + 620 + c.search_weight as score
    from name_word_hits h
    join public.marketplace_search_candidates c on c.card_id = h.card_id
    union all
    select c.card_id, h.query_token, h.token_score + 700 + c.search_weight as score
    from translated_name_hits h
    join public.marketplace_search_candidates c on coalesce(nullif(c.canonical_name, ''), c.name) = h.entity_key
  ),
  per_card_token as (
    select card_id, query_token, max(score) as token_score
    from token_blueprints
    group by card_id, query_token
  ),
  scored_cards as (
    select
      ...
      coalesce(sum(p.token_score), 0)
        ...
        + count(distinct p.query_token) * 420
        + case when count(distinct p.query_token) = (select count(*) from query_tokens) then 900 else 0 end as rank_score,
      count(distinct p.query_token) as matched_tokens
    from public.marketplace_search_candidates c
    join per_card_token p on p.card_id = c.card_id
    group by
...
create or replace function public.search_marketplace_blueprint_non_name_candidates(
  search_term text,
  result_limit integer default 20,
  result_offset integer default 0,
  search_language text default 'en'
...
as $$
  ...
  scored_cards as (
    select
      ...
      coalesce(sum(p.token_score), 0)
        + case
            when public.marketplace_search_normalize(coalesce(nullif(c.canonical_name, ''), c.name)) = (select q from normalized) then 1400
            else 0
          end
        + count(distinct p.query_token) * 360
        + case when count(distinct p.query_token) = (select count(*) from query_tokens) then 650 else 0 end
        + case when c.item_kind = 'product' then -80 else 0 end as rank_score,
      count(distinct p.query_token) as matched_tokens
    from public.marketplace_search_candidates c
    join per_card_token p on p.card_id = c.card_id
