-- English-named Japanese official sets that TCGDex already tagged tcgdex-ja
-- were left western: CardTrader pokemon_language is empty (WCP: 108 blueprints,
-- 0 jp / 0 en votes) so pokoin_expansion_nationality() falls through on the
-- English name. World Champions Pack is PCG10 ワールドチャンピオンズパック
-- (2007-07-05, 108 cards), not World Championship Decks (wcd*, western).
-- Same miss: Clash of the Blue Sky, Cry from the Mysterious, Holon Research
-- Tower, Miracle Crystal, Undone Seal.
...
as $fn$
  select case
    ...
    when lower(coalesce(code, '')) in (
      ...
      or coalesce(name, '') ~* 'movie commemoration|start deck 100|southern islands jp|constructed starter|half deck|battle master deck|battle strength deck|^world champions pack$'
      then 'japanese'
    else 'western'
  end;
$fn$;

-- Pin TCGDex JP/CN official_source before the language vote so English names
-- cannot flip them western. Do not match World Championship Decks.
...
set nationality = 'japanese',
    ...
    updated_at = now()
where official_source = 'tcgdex-ja'
