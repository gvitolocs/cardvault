-- L-P Promo (`l-ppromo`) is the Japanese LEGEND-era L-P Promotional cards
-- (メガニウム 001/L-P leftover). English name looks western; zero
-- pokemon_language votes → EU flag. English HGSS Black Star Promos stay
-- western. Do not regex '* Promo'.
...
set nationality = 'japanese',
    ...
    updated_at = now()
where (lower(code) = 'l-ppromo' or lower(name) = 'l-p promo')
...
where lower(code) in ('l-ppromo', 'hsp', 'hgssp')
   or name ~* '^l-p promo$|hgss black star'
order by name;
