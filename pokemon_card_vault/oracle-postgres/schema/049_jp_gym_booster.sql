-- Gym Booster 1 Leaders' Stadium (gls) and Gym Booster 2: Challenge from
-- the Darkness (gcd) are the Japanese Gym series (リーダーズスタジアム /
-- 闇からの挑戦). CardTrader's English names were aliased to pokemontcg.io
-- gym1 / gym2 (Gym Heroes / Gym Challenge). Zero pokemon_language votes →
-- western EUUS flag on Japanese scans.
--
-- English Gym Heroes stays `g1` / gym1. English Gym Challenge stays `g2` /
-- gym2. Do not pin those codes. `gym*` prefixes (Guren Town Gym, etc.) are
-- already japanese via the existing ^gym code regex; gcd/gls do not match it.
...
as $fn$
  select case
    ...
    when lower(coalesce(code, '')) in (
        ...
        'rog', 'l-ppromo', 'bw9', 'gcd', 'gls'
      )
      or coalesce(code, '') ~* '^(s[0-9]|sv[0-9]|sm[0-9]|m[0-9]|sp[0-9]|scjp|gym)'
      or coalesce(name, '') ~* 'movie commemoration|start deck 100|southern islands jp|constructed starter|battle master deck|battle strength deck|^world champions pack$|^awakening legends|gold, silver, to a new world|crossing the ruins|^rocket gang($| )|gym booster'
  ...
  with card as (
    ...
    select e.expansion_id,
      case
        ...
        when lower(e.code) in ('awl', 'awlpf', 'gstnw', 'n1pf', 'ctr', 'neojp', 'rog', 'l-ppromo', 'bw9', 'gcd', 'gls') then 'japanese'
...
set nationality = 'japanese',
    ...
    official_name = case
      when lower(code) = 'gls' then 'リーダーズスタジアム'
      when lower(code) = 'gcd' then '闇からの挑戦'
      else official_name
    end,
    listed = true,
    updated_at = now()
where lower(code) in ('gcd', 'gls');

select public.pokoin_refresh_expansion_nationality() as expansions_updated;

commit;

select code, name, nationality, milo_gallery, official_id, official_source, official_name, listed
from public.pokoin_pokemon_expansions
where lower(code) in ('gcd', 'gls', 'g1', 'g2')
   or lower(name) in (
     'gym booster 1 leaders'' stadium',
     'gym booster 2: challenge from the darkness',
     'gym heroes',
