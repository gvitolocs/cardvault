-- CS* CardTrader codes are mainland Chinese (025 comment). The classifier
-- only matched cs[mv0-9] / csv / csm / cbb, so letter suffixes after CS
-- (csdc, csf, csb, csgc, csh, csi, csuc) fell through to western.
-- Zero pokemon_language votes. Leftover faces are Simplified Chinese
-- (CSDC 皮卡丘, CSF 焰白酋雷姆). Widen to ^cs. Do not pin western sets.
...
as $fn$
  select case
    ...
    when coalesce(name, '') ~* 'simplified chinese|traditional chinese|gem pack|^collect 151'
      ...
      or lower(coalesce(code, '')) in ('151c', 'svp-c', '30thc', '30th-ch')
      then 'chinese'
  ...
  with card as (
    ...
    select e.expansion_id,
      case
        ...
        when lower(e.code) in ('awl', 'awlpf', 'gstnw', 'n1pf', 'ctr', 'neojp', 'rog', 'l-ppromo') then 'japanese'
...
set nationality = 'chinese',
    ...
    updated_at = now()
where lower(code) ~ '^(cs|cbb)'
  and nationality is distinct from 'chinese';
...
where lower(code) in ('csdc', 'csf', 'csb', 'csgc', 'csh', 'csi', 'csuc', 'cbb1c', 'cs1a')
order by code;
