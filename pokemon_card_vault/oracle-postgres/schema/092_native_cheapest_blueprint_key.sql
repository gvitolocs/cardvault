-- Native Pokoin listings compete with CardTrader on the same blueprint.
--
-- Before: eligible_native keyed rows by public card_id while CardTrader rows use
-- the CardTrader blueprint id, and public card_id = blueprint x2 (D00000B). So a
-- native offer never competed with CardTrader offers for the same card, and 975
-- native card ids (2026-09-24) collided with unrelated CardTrader blueprints of the
-- same number, overwriting that other card's cheapest row.
-- After: native rows key on card_id / 2 (odd ids, none today, go negative so they
-- can never collide). Same body as 089 otherwise.
...
begin
  ...
  eligible_native as (
    select
      ...
      native_listing.native_blueprint_id as blueprint_id,
    ...
    from (
      select
        ...
        -- Public card_id is the CardTrader blueprint x2 (D00000B): key native offers on that
        -- blueprint so they compete with CardTrader for the cheapest price instead of
        -- landing on an unrelated blueprint that happens to equal the card id.
        case
          when listing.card_id !~ '^[0-9]+$' then null
          when listing.card_id::bigint % 2 = 0 then listing.card_id::bigint / 2
          else -listing.card_id::bigint
        end as native_blueprint_id
      from public.marketplace_user_listings
      listing
    ) native_listing
    left join pg_temp.cardtrader_blueprint_listing_cache_scope scope
      on scope.blueprint_id = native_listing.native_blueprint_id
