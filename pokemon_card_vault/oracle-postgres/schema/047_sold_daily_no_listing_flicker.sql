-- Inferred CardTrader "sales" that repeat the same listing_id on many days are
-- snapshot flicker (listing left cheap-25 / incomplete book), not sold copies.
-- League stamp Milotic 257448 showed 446 units while ~20 asks were still live:
-- listing 448197265 qty 36 was archived inferred_sale eight days in a row.
-- Keep quantity_decreased (real stack drip). Keep inferred_sale only when that
-- listing_id appears on a single observed day.
