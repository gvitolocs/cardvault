-- Japanese Neo-era sets whose CardTrader *English* name was aliased to the
-- western pokemontcg.io id (neo1/neo2/neo3). Zero pokemon_language votes, so
-- pokoin_expansion_nationality() fell through to western and the set desk
-- showed the EU flag on Japanese scans.
--
-- Awakening Legends (awl) = めざめる伝説, 57 cards, 2000-11-23.
-- English Neo Revelation is n3 / neo3 (66 cards) and stays western.
-- Same alias: gstnw → neo1 (EN Neo Genesis is n1), ctr → neo2 (EN Neo Discovery is n2).
-- Do not pin n1/n2/n3/n4 or wcd*.
...
as $fn$
  select case
    ...
    when lower(coalesce(code, '')) in (
      ...
      or coalesce(name, '') ~* 'movie commemoration|start deck 100|southern islands jp|constructed starter|half deck|battle master deck|battle strength deck|^world champions pack$|^awakening legends|gold, silver, to a new world|crossing the ruins'
      then 'japanese'
    else 'western'
  end;
$fn$;
  ...
  with card as (
    ...
    select e.expansion_id,
      case
        ...
        when lower(e.code) in ('awl', 'awlpf', 'gstnw', 'n1pf', 'ctr', 'neojp') then 'japanese'
...
set nationality = 'japanese',
    ...
    updated_at = now()
where lower(code) in ('awl', 'awlpf', 'gstnw', 'n1pf', 'ctr', 'neojp')
...
where lower(code) in ('awl', 'awlpf', 'gstnw', 'n1pf', 'ctr', 'neojp', 'n1', 'n2', 'n3', 'n4', 'del')
order by code;
