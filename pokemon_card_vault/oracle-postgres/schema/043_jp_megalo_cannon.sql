-- Megalo Cannon (`bw9`) is Japanese BW9 メガロキャノン. Sanitize aliased it
-- to tcgdex-en Plasma Freeze (official_id=bw9), zero pokemon_language votes,
-- so pokoin_expansion_nationality() fell through to western and the set desk
-- showed the EUUS flag on Japanese scans.
--
-- English Plasma Freeze stays `plf` / pokemontcg.io bw9. Do not pin plf.
...
set nationality = 'japanese',
    ...
    official_id = 'BW9',
    official_name = 'メガロキャノン',
    official_source = 'tcgdex-ja',
    listed = true,
    updated_at = now()
where lower(code) = 'bw9'
  and lower(name) = 'megalo cannon';

select public.pokoin_refresh_expansion_nationality() as expansions_updated;

commit;

select code, name, nationality, milo_gallery, official_id, official_source, official_name
from public.pokoin_pokemon_expansions
where lower(code) in ('bw9', 'plf')
   or lower(name) in ('megalo cannon', 'plasma freeze')
order by code;
