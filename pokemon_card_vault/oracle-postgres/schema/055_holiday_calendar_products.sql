-- CardTrader files sealed Holiday Calendars, their keychains and sticker
-- sheets inside the card expansion. They have no printed collector fraction
-- and must stay in Products, while the stamped cards remain in Cards.
...
as $$
  ...
  select case
    ...
    then 'card'
    when name ~ '(^|[^a-z0-9])(coin|sleeves|sleeve|playmat|binder|portfolio|divider|dividers|accessory|bag|shoulder bag|backpack|keychain|sticker sheet)([^a-z0-9]|$)'
      or category ~ '(^|[^a-z0-9])(coin|sleeves|sleeve|playmat|binder|portfolio|divider|dividers|accessory|bag|shoulder bag|backpack|keychain|sticker sheet)([^a-z0-9]|$)'
      or type ~ '(^|[^a-z0-9])(coin|sleeves|sleeve|playmat|binder|portfolio|divider|dividers|accessory|bag|shoulder bag|backpack|keychain|sticker sheet)([^a-z0-9]|$)'
    then 'accessory'
    ...
    then 'tin'
    when name ~ '(^|[^a-z0-9])(premium collection|special collection|collection box|box set|gift box|card frame box|frame box|collection|collector.?s? chest|empty mini|blister|case|toolkit|figure|pin|holiday calendar)([^a-z0-9]|$)'
...
set
  product_type = case
    when name ~* 'keychain|sticker sheet' then 'accessory'
    else 'collection_box'
  end,
  item_kind = 'product'
where lower(set_name) = 'holiday calendar'
  and coalesce(card_number, '') !~ '[0-9]{1,4}[A-Za-z]?/[0-9]{1,4}'
  and name ~* 'keychain|sticker sheet|^pok(é|e)mon tcg: holiday calendar';
...
where lower(expansion_name) = 'holiday calendar'
