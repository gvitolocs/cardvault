-- Eevee GX Starter Sets (`sml`) is the Japanese SM exclusive trio
-- スターターセット「炎のブースターGX」「水のシャワーズGX」「雷のサンダースGX」
-- (TCGDex SMI, 2018-11-23). CardTrader's English name has zero
-- pokemon_language votes → western EUUS flag on Japanese scans.
--
-- Do not regex `starter set`: English XY Kalos Starter Set (`kss`) stays
-- western. Do not pin Chinese Eevee GX gift boxes (`csh` / `csmy`).
  ...
  with card as (
    ...
    select e.expansion_id,
      case
        ...
        when lower(e.code) in ('awl', 'awlpf', 'gstnw', 'n1pf', 'ctr', 'neojp', 'rog', 'l-ppromo', 'bw9', 'gcd', 'gls', 'sml') then 'japanese'
...
set nationality = 'japanese',
    ...
    official_id = coalesce(nullif(official_id, ''), 'SMI'),
    official_name = coalesce(nullif(official_name, ''), 'スターターセット「炎のブースターGX」「水のシャワーズGX」「雷のサンダースGX」'),
    updated_at = now()
where lower(code) = 'sml'
   or lower(name) = 'eevee gx starter sets';

select public.pokoin_refresh_expansion_nationality() as expansions_updated;

commit;

select code, name, nationality, milo_gallery, official_id, official_source, official_name, kind, listed
from public.pokoin_pokemon_expansions
where lower(code) in ('sml', 'kss', 'csh', 'csmy')
   or lower(name) in (
     'eevee gx starter sets',
