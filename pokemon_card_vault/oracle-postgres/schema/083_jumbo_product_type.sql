-- 083: Jumbo Oversized becomes its own Pokémon product type.
-- CardTrader category 78 (Pokémon Oversized / jumbo card) mapped to
-- product_type 'jumbo' instead of collapsing into 'card'. Jumbos stay
-- item_kind 'single' (they are cards: scannable, pokedex-sorted, artist-
-- attributed) but leave the generic card bucket so prices, home rails,
-- and deck lookups stop treating them like standard printings.
-- Idempotent: create-or-replace + re-derive via apply_cardtrader_category_kind().
...
AS $function$
  select case category_id
    ...
    when 78 then 'jumbo'             -- Pokémon Oversized (jumbo card): its own Pokémon product type
...
begin
  ...
  set
    ...
    item_kind = case when r.product_type in ('card', 'jumbo') then 'single' else 'product' end,
    projected_at = now()
  ...
  where b.id = c.ct_id
    ...
    and (
      ...
      or c.item_kind is distinct from (case when r.product_type in ('card', 'jumbo') then 'single' else 'product' end)
    );
...
begin
  ...
  select
    ...
    source.rarity,
    source.card_type,
    case when source.product_type in ('card', 'jumbo') then coalesce(source.explicit_card_number, source.version, source.id::text) else coalesce(source.explicit_card_number, '') end,
    lower(coalesce(source.rarity, '')) like '%holo%',
    lower(coalesce(source.rarity, '')) like '%holo%',
    source.imported_at,
    now(),
    case when source.product_type in ('card', 'jumbo') then 'single' else 'product' end,
    source.product_type,
    source.trainer_name,
    source.card_palette,
    source.emoji
...
-- Re-derive marketplace_cards -> candidates -> versions from the patched mapping.
