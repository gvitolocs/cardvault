...
PokoinPoS supports native NFTs directly in the chain runtime. These are not
ERC-721 or ERC-1155 smart contracts; they are first-class Pokoin ledger objects
validated by the chain and exposed through node APIs.

## Current Status

The public RPC node exposes native NFT inventory at:

```text
GET https://rpc.pokoin.com/chain/nfts
```

When no NFTs are minted, the response is:
...
NFT support is deployed on the public RPC node through:

```text
newisdom/pokoinpos-peer:latest
```

MetaMask remains supported for PKN balances and transfers only. NFT inventory
should be displayed through Pokoin, Card Vault, or explorer UI using the native
NFT APIs.
...
The chain should store only ownership plus metadata pointer/hash. Full metadata
should live in Card Vault or IPFS and include:
...
- Metadata update transactions are not implemented yet, so minted metadata should
  be treated as immutable.
- Transfers are currently operator/admin controlled unless wallet-signed native
  NFT transfer support is added later.
