alter table public.marketplace_cards
  ...
  check (item_kind in ('single', 'product'));

create index if not exists marketplace_cards_item_kind_idx
  on public.marketplace_cards (item_kind, imported_at desc nulls last);
...
begin
  ...
  select
    ...
    now(),
    case
      when lower(coalesce(b.name, '')) ~ '(^|[^a-z0-9])(booster|box|pack|bundle|tin|deck|etb|elite trainer|collection|display|case|blister|starter|theme deck)([^a-z0-9]|$)'
        or lower(coalesce(b.blueprint->>'category_name', '')) ~ '(^|[^a-z0-9])(sealed|booster|box|pack|deck|product)([^a-z0-9]|$)'
        or lower(coalesce(b.blueprint->>'type', '')) ~ '(^|[^a-z0-9])(sealed|booster|box|pack|deck|product)([^a-z0-9]|$)'
      then 'product'
      else 'single'
    end as item_kind
...
as $$
  ...
  select jsonb_build_object(
    'generatedAt', now(),
    'window', jsonb_build_object('lookbackHours', 24, 'persistentSpotlight', false),
    'cards', coalesce(jsonb_agg(jsonb_build_object(
      ...
      'releaseDate', now(),
      'tags', jsonb_build_array(cards.set_name, cards.rarity, cards.card_type, cards.item_kind),
      'condition', 'NM',
      'isGraded', false,
