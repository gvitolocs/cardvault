...
as $$
  ...
  signals as (
    select
      *,
      ...
      number ~ '(^|[^0-9])[0-9]{1,4}[a-z]?/[0-9]{1,4}([^0-9]|$)' as has_collector_number,
  ...
  select case
    ...
    then 'booster_bundle'
    when name ~ '(^|[^a-z0-9])(booster pack|booster|pack)([^a-z0-9]|$)'
      or category ~ '(^|[^a-z0-9])(booster pack|booster|pack)([^a-z0-9]|$)'
      or type ~ '(^|[^a-z0-9])(booster pack|booster|pack)([^a-z0-9]|$)'
    then 'booster_pack'
